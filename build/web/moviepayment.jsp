			...
			<div class="col-md-8 payment-left">
				...
				<div class="payment-options">
					...
					<div class="tab-grids event-tab-grids">
                                            <form action="final.jsp?sid=<%=sid%>&&time=<%=time%>&&amount=<%=amount%>&&seat=<%=seat%>&&qty=<%=qty%>" method="post">
						<div id="tab1" class="tab-grid">
							<img src="images/payment.png" alt="payment" />
							<input type="text" name="cardno" class="payment" value="Enter Your Card Number" onfocus="this.value = '';" onblur="if (this.value == '') {this.value = 'Enter Your Card Number';}">
							<input type="text" name="cardname" class="payment" value="Your Name On Card" onfocus="this.value = '';" onblur="if (this.value == '') {this.value = 'Your Name On Card';}">
							<p>Expiry:<select class="month" name="month">
...
%>
				<a href="index.jsp" class="go to pay">PLEASE LOGIN</a>
					
				 <%} else {
%>
									<a>	<input type="submit" class="go-to-pay" value="make payment">
<%
    }
%>
    ...
    ResultSet rsplay=st.executeQuery("select TIME_FORMAT(sdate,'%W, %d %M, %yyyy'),tid,mid from movieshow where sid="+sid);
rsplay.next();    
	Statement sttf = con.createStatement();
	ResultSet rsplaym=sttf.executeQuery("select * from movie where mid='"+rsplay.getString("mid")+"'");
rsplaym.next();    
	Statement sttt = con.createStatement();
	ResultSet rspaymt=sttt.executeQuery("select * from theatre where tid='"+rsplay.getString("tid")+"'");
rspaymt.next();    
	...
	%>     
			...
			<div class="payment-right">
				...
				<h6><span>Movie-Name:</span><%=rsplaym.getString("mname")%></h6>
				<p><span>Theatre:</span><%=rspaymt.getString("tname")%></p>
				<p><span>Area:</span><%=rspaymt.getString("area")%></p>
				<p><span>Location:</span> Mumbai</p>
				<p><span>Date:</span> Sat, 8 Aug, 2015</p>
				<p><span>Time:</span> <%=time%></p>
				<p><span>Seat Info:</span> <%=seat%></p>
				<p><span>Qty:</span> <%=qty%></p>
				<h4><span>Total :</span>Rs. <%=amount%></h4>
